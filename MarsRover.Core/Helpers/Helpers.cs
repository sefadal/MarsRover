...
namespace MarsRover.Core.Helpers
{
    public static class Helpers
    {
        ...
        public static async Task<IList<IExecutionResult>> PublishMultipleAsync(this ICommandBus commandBus, ICommand[] commands, bool continueOnError = false)
        {
            if (!commands.Any())
                return null;
        ...
        public static IEnumerable<ICommand> ToRoverCommands(this string input, Identity roverId)
        {
            foreach (char letter in input)
            {
                ...
                {
                    case 'L':
                        yield return new LeftCommand(roverId);
                        break;

                    case 'R':
                        yield return new RightCommand(roverId);
                        break;
